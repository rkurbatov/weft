...
**The outbox is three things, not one.** `book.ts` is the writing down: one writer, versions coalescing while a write is in flight, the lift of whatever a previous run left, and a refused write shown as a state rather than a silence — it knows nothing of clocks or handlers. `schedule.ts` is the waiting: doubling with a cap, one timer at a time, held and released — it knows nothing of books. `outbox.ts` is what is left and what was always the hard part: what a refusal means, which faults are worth repeating, and the order in which entries go out. Each has its own tests, and the two smaller ones need neither a disk nor a clock to be tested.

**The wire is three layers, not one.** `transport.ts` carries messages and knows nothing else: four methods — post, on, close — over a browser's `BroadcastChannel`, Node's own, or a set of callbacks. `postbox.ts` adds addressing: who a letter is from, who it is for, and the greeting by which an arrival makes itself known; what is inside an envelope is none of its business. `bus.ts` is the protocol proper: who is served, for how long, how a lease is renewed and when a session is refused. The whole tab protocol is therefore exercised in tests over an in-memory line — no browser, no worker threads — and a new transport costs four methods.
