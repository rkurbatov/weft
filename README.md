...
**`src/weft/core/graph.ts` — the graph.** Stored cells (`input`) with exactly one writer each; derived cells (`cell`) whose dependencies come from reading, never from a declared list; watchers (`watch`, `subscribe`); write batches (`batch`).
...
**`src/weft-react` — native hooks,** a unit of its own for whoever takes the engine directly: `useCell`, `useCommand`, `useSource`, `useInputBinding`, `useSourceValue`. Deliberately thin, imports the engine only through its front door. Loom takes `useField` from here rather than rewriting the mechanics; the convenient layer's own seam is `src/loom/react.ts`.
...
What is not done, and should be said plainly. No live application has been moved onto this yet — the demos are stands, and the honest next step is one domain of a real product, whole. The convenient layer has no word for a delta-fed collection, so the rail stand reaches for the engine's table directly — the one leak it promised not to have.
