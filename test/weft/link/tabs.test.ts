...
import { leadOrFollow, webLocks } from '#weft/link/lead.ts'
import type { Lock } from '#weft/link/lead.ts'
...
test('the shared-worker hub lets go of a tab that fell silent', async () => {
  ...
  const stopHub = sharedWorkerHub(scope, { lease: 15_000, timers: clock.timers }).accept(channel =>
    serve(surface, channel, { schedule: atOnce }),
  )
