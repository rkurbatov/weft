...
for (let row = 0; row < VISIBLE; row++)
  for (let col = 0; col < shape.cols; col++) onScreen.push(key(row, col));

console.log(
  `sheet ${shape.rows}x${shape.cols} = ${sizeOf(shape).toLocaleString()} cells, ${onScreen.length} of them on screen\n`,
);

{
  ...
  console.log(
    `classic   build ${built.toFixed(0)}ms | edit A1 ${edit.toFixed(1)}ms | recomputed ${sheet.recomputes()} | cells told ${told}`,
  );
}

{
  ...
  console.log(
    `on weft   lay out ${laid.toFixed(0)}ms + first look ${firstPaint.toFixed(0)}ms (${worked} cells) | edit A1 ${edit.toFixed(1)}ms | recomputed ${sheet.recomputes()} | cells told ${told}`,
  );
