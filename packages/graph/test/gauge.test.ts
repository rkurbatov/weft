...
import { batch, gauge, derived, graph, port, subscribe, watch } from '#graph'
...
describe('counters over the graph', () => {
  test('ticks, recomputes and wakings are counted', () => {
    ...
    assert.equal(count.counts.ticks.peek(), 2, 'two settlings')
    assert.equal(count.counts.computed.peek(), 2, 'the formula ran once per write')
    assert.equal(count.counts.woken.peek(), 2, 'and the watcher woke twice')
  })
  ...
  test('a batch is one tick, whatever it holds', () => {
    ...
    assert.equal(count.counts.ticks.peek(), 1, 'fifty writes, one settling')
    assert.equal(count.counts.computed.peek(), 1, 'and one recompute')
  })
  ...
  test('a recompute that changes nothing is counted as gated', () => {
    ...
    assert.equal(count.counts.gated.peek(), 1)
    assert.equal(count.counts.woken.peek(), 0, 'nobody below was woken')
  })
  ...
  test('a failure is counted, not swallowed', () => {
    ...
    engine.build(() => {
      ...
      const count = gauge({ engine: engine })
      until(count.stop)
      ...
      assert.equal(count.counts.failed.peek(), 2)
    })
  })
  ...
  test('watchers alive are counted, and the queue is at its deepest', () => {
    ...
    seats.set(2)
    assert.equal(count.counts.watching.peek(), 3, 'three screens')

    for (const stop of stops) stop()
    seats.set(3)
    assert.equal(count.counts.watching.peek(), 0)
  })
  ...
  test('the counters are cells: a formula may depend on one', () => {
    ...
    const busy = derived(() => count.counts.ticks.get() > 1, { name: 'busy' })
  ...
  test('counting one graph does not count another', () => {
    ...
    assert.equal(count.counts.ticks.peek(), 0, 'the other engine settles on its own')
    here.set(2)
    assert.equal(count.counts.ticks.peek(), 1)
  })
  ...
  test('stopping costs nothing afterwards', () => {
    ...
    const count = gauge()
    seats.set(2)
    const settled = count.counts.ticks.peek()

    count.stop()
    seats.set(3)
    seats.set(4)
    assert.equal(count.counts.ticks.peek(), settled, 'nothing is counted once the probe is gone')
  })
})

test('counters and the journal are on at once, which is the whole point', () => {
  // Before the gauge these were two doors to one probe, and attaching the
  // second silently unhooked the first: a panel showing both showed one of
  // them frozen at whatever it held. Now one attachment feeds both.
  const seats = port(1, { name: 'gauge.both.seats' })
  const doubled = derived(() => seats.get() * 2, { name: 'gauge.both.doubled' })
  const readings = gauge()
  until(subscribe(doubled, () => {}))

  batch(() => seats.set(2))
  batch(() => seats.set(3))

  assert.equal(readings.counts.ticks.peek() > 0, true, 'the counters moved')
  assert.equal(readings.log.ticks().length > 0, true, 'and the log filled at the same time')
  assert.equal(readings.log.why('gauge.both.doubled') !== undefined, true)
  readings.stop()
})
