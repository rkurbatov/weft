...
import { keepInput, keepSupply } from '#store'
import { memoryStore } from '#store'
import { supply } from '#remote'
import type { Dropped } from '#store'
...
describe('keeping things on disk', () => {
  ...
  test('keeping a source does not ask it for anything', async () => {
    ...
    const kept = keepSupply(feed, { key: 'feed', store, now: clock.now })
    await kept.restored
    await settle()
  ...
  test('an answer kept from the last run comes back with its real age', async () => {
    ...
    const kept = keepSupply(feed, { key: 'feed', store, now: first.now })
    ...
    let laterCalls = 0
    const revived = supply(async () => `answer ${++laterCalls}`, {
      shelfLife: 5000,
      now: soon.now,
      timers: soon.timers,
    })
    const keptAgain = keepSupply(revived, { key: 'feed', store, now: soon.now })
  ...
  test('past its shelf life the restored answer is shown and refreshed at once', async () => {
    ...
    const keptLate = keepSupply(revived, { key: 'feed', store, now: late.now })
  ...
  test('the network answering before the disk wins: nothing is put over a live answer', async () => {
    ...
    const clock = world()
    const feed = supply(async () => 'from the world', { now: clock.now, timers: clock.timers })
    const stop = subscribe(feed.state, () => {})
    await settle()
    assert.equal(feed.state.peek().value, 'from the world')

    const kept = keepSupply(feed, { key: 'feed', store: disk.store, now: clock.now })
